CREATE TABLE hai_az_prod.lab_scc_2020July AS
with
	lab_flat as (
		SELECT dp.medical_record_number , f.person_key , f.encounter_key , f.caregiver_group_key , f.facility_key , f.operation_key, dc.calendar_date :: Date , f.time_of_day_key,
		...
		JOIN hai_az_prod.fd_procedure fdp on bp.procedure_key = fdp.procedure_key
		...
		GROUP BY dp.medical_record_number , f.person_key , f.encounter_key , f.caregiver_group_key , f.facility_key , f.operation_key, dc.calendar_date , f.time_of_day_key,
		bp.procedure_group_key , bp.procedure_rank , bp.procedure_key , fdp.context_name , fdp.context_procedure_code , fdp.procedure_description ,
		dm.level1_context_name , dm.level2_event_name , dm.level3_action_name
	)
SELECT medical_record_number , encounter_key , facility_key , calendar_date :: Date, time_of_day_key,
